...
export default async function ArtistPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!result) notFound();
  return <GalleryShell><main><section className="relative isolate min-h-[62vh] overflow-hidden px-5 py-24 sm:px-10 lg:px-16"><Image src={result.artist.image} alt="" fill priority sizes="100vw" className="-z-20 object-cover opacity-45" /><div className="absolute inset-0 -z-10 bg-gradient-to-t from-ink via-ink/60 to-black/10" /><div className="flex min-h-[45vh] flex-col justify-end"><p className="eyebrow">{result.artist.location} · {result.artist.discipline}</p><h1 className="section-title mt-6">{result.artist.name}</h1><p className="mt-5 max-w-2xl text-lg leading-relaxed text-white/65">{result.artist.bio}</p><div className="mt-6 flex gap-6 text-sm text-white/50"><span>{result.artist.followerCount} followers</span><span>{result.artist.works} artworks</span><span>{result.artist.rating.toFixed(1)} rating</span></div></div></section><section className="section-shell py-20"><h2 className="font-serif text-5xl">All artwork</h2><div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{result.artworks.map((artwork) => <Link href={`/artwork/${artwork.slug}`} key={artwork.id} className="group"><div className="relative aspect-[4/5] overflow-hidden"><Image src={artwork.image} alt={artwork.title} fill sizes="(max-width: 640px) 100vw, 33vw" className="object-cover transition duration-700 group-hover:scale-[1.025]" /></div><div className="mt-4 flex justify-between gap-4"><div><h3 className="font-serif text-2xl">{artwork.title}</h3><p className="text-sm text-white/45">{artwork.year} · {artwork.medium}</p></div><span className="text-sm font-semibold">{formatPrice(artwork.price)}</span></div></Link>)}</div></section></main></GalleryShell>;
}
