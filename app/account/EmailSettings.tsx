...
function StatusMessage({
  ...
  state: EmailVerificationActionState;
}) {
  ...
  const challengeActive = Boolean(
    eventId &&
      ...
      !(
        ...
        verifyState.eventId === eventId
      ) &&
      cancelState.cancelledEventId !== eventId,
  );

  const displayedState =
    cancelState.cancelledEventId &&
    cancelState.cancelledEventId === eventId
      ? cancelState
    ...
    <div className="mt-6 border-t border-white/10 pt-6">
      ...
      !(
        ...
        <form action={requestAction} className="mt-6">
          ...
          >
            Verify or change contact email
          </label>

          <p className="mt-1 text-xs leading-relaxed text-white/40">
            Your Google sign-in email stays unchanged. Enter a different
            contact email below. The new address becomes active only after
            verification.
          </p>
          ...
          <div className="mt-3 flex flex-col gap-3 sm:flex-row">
            <input
              ...
              autoComplete="off"
              defaultValue=""
              placeholder="Enter a different contact email"
              required
              maxLength={320}
              disabled={requestPending}
              className="min-h-12 flex-1 border border-white/15 bg-transparent px-4 text-sm text-white outline-none transition placeholder:text-white/25 focus:border-white/60 disabled:opacity-50"
            />

            <button
              type="submit"
              disabled={requestPending}
              className="button-light disabled:cursor-wait disabled:opacity-60"
            >
      ...
      {challengeActive && eventId && pendingEmail ? (
        <div className="mt-6 border border-white/10 bg-white/[.025] p-5">
          <p className="text-sm text-white/75">
            ...
            <span className="break-all text-white">{pendingEmail}</span>.
          </p>

          <p className="mt-1 text-xs text-white/40">
            The six-digit code expires in 10 minutes and can be used once.
          </p>
          ...
          <form action={verifyAction} className="mt-5">
              ...
              <InputOTPGroup>
                {Array.from({ length: 6 }, (_, index) => (
                  <InputOTPSlot
                    key={index}
                    index={index}
                    className="h-11 w-10 border-white/20 bg-black/20 text-base sm:w-12"
                  />
                ))}
              </InputOTPGroup>
            </InputOTP>
            ...
            <button
              type="submit"
              disabled={verifyPending}
              className="button-light mt-5 disabled:cursor-wait disabled:opacity-60"
            >
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <form action={requestAction}>
              ...
              <button
                ...
                className="text-link text-xs disabled:cursor-not-allowed disabled:opacity-40"
              >
                {cooldownSeconds > 0
            ...
            <form action={cancelAction}>
              ...
              <button
                ...
                className="text-xs text-white/45 underline-offset-4 hover:text-white hover:underline disabled:opacity-40"
              >
                {cancelPending
                  ? "Cancelling…"
